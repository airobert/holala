...
(*                                                                           *)
(*            (c) Copyright, Sanaz Khan Afshar & Vincent Aravantinos 2011-12 *)
...
let RE_IM_NORM = prove
  ...
  THEN (MATCH_MP_TAC COMPONENT_LE_NORM
    ORELSE MATCH_MP_TAC COMPONENT_LE_NORM_ALT)
  THEN REWRITE_TAC[DIMINDEX_2] THEN ARITH_TAC);;
...
let CSQRT_MUL_LCX_LT = prove
  ...
  THENL [
    ...
    THENL [
      MATCH_MP_TAC SQRT_DIV THEN CONJ_TAC THENL [
    ...
    THENL [
      MATCH_MP_TAC SQRT_DIV THEN CONJ_TAC
...
let COMPLEX_VECTOR_MAP = prove
  (`!f:complex->complex g. f = vector_map g <=> !z. f z = complex (g (Re z), g (Im z))`,
...
let COMPLEX_NEG_IS_A_MAP = prove(`(--):complex->complex = vector_map ((--):real->real)`, REWRITE_TAC[COMPLEX_VECTOR_MAP;complex_neg]);;

let VECTOR_NEG_IS_A_MAP = prove(`(--):real^N->real^N = vector_map ((--):real->real)`,
  REWRITE_TAC[FUN_EQ_THM;CART_EQ;VECTOR_NEG_COMPONENT;VECTOR_MAP_COMPONENT]);;

let VECTOR_MAP_VECTOR_MAP_ALT = prove(`!f:A^N->B^N g:C^N->A^N f' g'. f = vector_map f' /\ g = vector_map g' ==> f o g = vector_map (f' o g')`,
  SIMP_TAC[o_DEF;FUN_EQ_THM;VECTOR_MAP_VECTOR_MAP]);;

let COMPLEX_VECTOR_MAP2 = prove(
  `!f:complex->complex->complex g. f = vector_map2 g <=> !z1 z2. f z1 z2 = complex (g (Re z1) (Re z2), g (Im z1) (Im z2))`,
  REWRITE_TAC[vector_map2;FUN_EQ_THM;complex] THEN REPEAT (GEN_TAC ORELSE EQ_TAC)
  THEN ASM_SIMP_TAC[CART_EQ;DIMINDEX_2;FORALL_2;LAMBDA_BETA;VECTOR_2;RE_DEF;IM_DEF]);;
...
(* TODO
let CLINEAR_BOUNDED = prove
 (`!f:complex^M->complex^N. clinear f ==> ?B. !x. norm (f x) <= B * norm x`,
 
  REPEAT STRIP_TAC
  THEN EXISTS_TAC `sum(1..dimindex(:M)) (\i. norm((f:complex^M->complex^N)
    (vector_to_cvector (basis i))))` THEN
  GEN_TAC THEN
...
(* TODO
 *let CNORM_TRIANGLE_EQ = prove(`!x y:complex^N. norm (x + y) = norm x + norm y <=> Cx(norm x) % y = Cx(norm y) % x`,
 *  REPEAT GEN_TAC THEN ASM_CASES_TAC `x=cvector_zero:complex^N`
 *  THEN ASM_REWRITE_TAC[CNORM_CVECTOR_ZERO;CVECTOR_ADD_LID;REAL_ADD_LID;CVECTOR_MUL_LZERO;CVECTOR_MUL_RZERO]
 *  THEN SIMP_TAC[CNORM_POS;REAL_ARITH `&0 <= x /\ &0 <= y /\ &0 <= z ==> (x = y + z <=> abs x = abs (y+z))`;REAL_EQ_SQUARE_ABS;cnorm;o_DEF;CNORM_POW_2;
 *    REAL_ADD_POW_2;cnorm2;CDOT_LADD;CDOT_RADD;SIMPLE_COMPLEX_ARITH `(x+y)+z+t = x+(y+z)+t:complex`;ADD_CDOT_SYM;REAL_ADD;REAL_CDOT_SELF;REAL_CX;
 *    REAL_OF_COMPLEX_ADD;REAL_OF_COMPLEX_CX;REAL_ARITH `x+ &2*y+z=x+z+ &2*t <=> y=t:real`]
 *)
 ...
 * (`!x:complex^N. vsum(1..dimindex(:N)) (\i. x$i % basis i) = x`,
...
let LINEAR_COMPOSE_VSUM = prove
 (`!f s. FINITE s /\ (!a. a IN s ==> clinear (f a))
         ==> clinear (\x. vsum s (\a. f a x))`,
