...
(* ------------------------------------------------------------------------- *)
(* Definition of FORALL_DEF and IMP_DEF                                      *)
(* ------------------------------------------------------------------------- *)


logfile "bool-def";; 

let FORALL_DEF = prove 
  (`(!) = \P. P = \x.T`,
...
let IMP_DEF = prove
  ( `(==>) = (\p q. p /\ q <=> p)`,
    REWRITE_TAC [FUN_EQ_THM] THEN REPEAT GEN_TAC THEN ITAUT_TAC 
  );;
...
let EXCLUDED_MIDDLE = let thm =prove
 (`!t. t \/ ~t`,
  ...
  MP_TAC THENL
   [CONJ_TAC THEN CONV_TAC SELECT_CONV THENL
    ...
    ASM_REWRITE_TAC[ITAUT `p \/ T <=> T`]]) in 
  let () = replace_proof thm Lem_proof in 
  thm ;;
